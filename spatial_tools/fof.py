...
def get_fof_PR(positions, test_result, scale, fofid = None):
    ...
    extended_fofid: np.array
        FOF IDs of extended groups including non-infected friends
    mean_pr_ext_fof: np.array
        Mean PR corresponding to extended_fofid
    """
    ...
    #Mean PR for all positions cases in extended FOFs
    mean_pr_ext_fof = np.zeros_like(test_result)
    #ID of the extended FOFs
    extended_fofid = np.zeros_like(test_result)
    for i,f in enumerate(np.unique(fofid[fofid>0])):
    ...
    return fofid, mean_pr_fof, extended_fofid, mean_pr_ext_fof
